( function () {
    ...
    window.ReviewDashboard = {
        ...
        init: function () {
            // Set default URLs - UPDATED for HTTPS
            document.getElementById('sparql-endpoint').value = 'https://wikibase.local/extensions/ReviewDashboard/api/sparql-proxy.php'; // Changed to https
            document.getElementById('wikibase-url').value = 'https://wikibase.local'; // Changed to https
            ...
            document.getElementById('confirm-add-label-btn').addEventListener('click', this.confirmAddLabel.bind(this)); // Added this line, was missing
        ...
        loadItems: function () {
            ...
            const sparqlQuery = `
                ...
                ORDER BY ?itemLabel
            `;
            // NOTE: The SPARQL query itself *can* use http://wikiba.se/ontology# as that's a namespace URI,
            // but the `wdt:P2` prefix should ideally use the actual Wikibase instance's URL for consistency.
            // However, since it's a URI, it doesn't cause mixed content. Keep 'https://wikibase.local/prop/direct/' for consistency.
    ...
    resultsSection.style.display = 'block';
        ...
        performAddLabel: async function (itemId, language, labelText) {
            // Get edit token - UPDATED for HTTPS
            const tokenResponse = await fetch('https://wikibase.local/w/api.php?action=query&meta=tokens&format=json'); // Changed to https
            const tokenData = await tokenResponse.json();
            const token = tokenData.query.tokens.csrftoken;

            // First, check if the item already has a label in this language - UPDATED for HTTPS
            const checkResponse = await fetch(`https://wikibase.local/w/api.php?action=wbgetentities&ids=${itemId}&format=json`); // Changed to https
            ...
            if (hasExistingLabel) {
                ...
                // Add as alias - UPDATED for HTTPS
                const response = await fetch('https://wikibase.local/w/api.php', { // Changed to https
                    method: 'POST',
                ...
                // Add as label (no existing label) - UPDATED for HTTPS
                const response = await fetch('https://wikibase.local/w/api.php', { // Changed to https
                    method: 'POST',
                    headers: { 'Content-Type': 'application/x-www-form-urlencoded' },
                    body: new URLSearchParams({
